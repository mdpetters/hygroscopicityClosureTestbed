...
begin
	...
	All concentrations are in μg m⁻³. CV is the coefficient of variation for σᵢ/μᵢ. n is the number of simulations.
	
	| Problem Setup      | Cations | Anions | Other | Foo | 
	| ----------- | ----------- | ----------- | ----------- | ----------- |
	| CV : $(aCV)    | NH₄⁺: $(aNH4)| SO₄²⁻: $(aSO4)	| Seasalt: $(ass) | NH₄⁺: $(aNH4) |
	| n: $(an) | | NO₃⁻: $(aNO3)| OA: $(aOA)	 | |
...
function plot_AIOMFAC(df)
	set_default_plot_size(4inch, 2.5inch)
	ylabels = [0, 0.5, 1]
...
function evaluate(NH4, SO4, NO3, nrCl, ss, OA, rBC)
	...
	ρd = (OAM + SO4M + NO3M + NH4M + nrClM + ssM + rBCM)/
		(OAM/1.7 + (SO4M + NO3M + NH4M)/1.75 + nrClM/1.52 + ssM/1.45 + rBCM/1.77)
...
function monte_carlo(compounds, MWs)
	ϵ = 1e-9
	pool = create_pool(
		rand(Uniform(0, NH4*CV+ϵ)), 
		rand(Uniform(0, SO4*CV+ϵ)), 
		rand(Uniform(0, NO3*CV+ϵ)), 
		rand(Uniform(0, nrCl*CV+ϵ)),
		rand(Uniform(0, ss*CV+ϵ))
	)
...
begin
	finorg, fOA, frBC, ρd = evaluate(NH4, SO4, NO3, nrCl, ss, OA, rBC)
	md"""
...
begin 
	km = finorg*km1 + fOA*measurement(0.05, 0.05) 
	kv = km*ρd
...
begin
	OAs = 0:0.1:300
	kv1 = map(OAs) do x
		fi1, fOA1, frBC1, ρd1 = evaluate(NH4, SO4, NO3, nrCl, ss, x, rBC)
		kmo = fi1*km1 + fOA1*measurement(0.05, 0.05) 
		kmo*ρd1
	end
	...
	lvals1 = mitp1(xvals)
	i1 = argmin((lvals1 .- 0.5).^2)
	mitp2 = interpolate((xvals,),vals .- err, Gridded(Linear()))
	lvals2 = mitp2(xvals)
	i2 = argmin((lvals2 .- 0.5).^2)
	

	j = argmin((vals .- 0.3).^2)
	j1 = argmin((lvals1 .- 0.3).^2)
	j2 = argmin((lvals2 .- 0.3).^2)
	...
	p2 = plot(
		...
		Geom.ribbon, 
		Geom.line),
		layer(x = [xvals[i]], y = [vals[i]], xmax = [xvals[i1]], xmin = [xvals[i2]],
			Geom.point, 
			Geom.xerrorbar, 
			Theme(default_color = "steelblue3")
		),
		layer(x = [xvals[j]], y = [vals[j]], xmin = [xvals[j1]], xmax = [xvals[j2]],
			Geom.point, 
			Geom.xerrorbar, 
			Theme(default_color = "steelblue3")
		),
	...
	**External Constraint on OA mass**
	...
	Assuming that kv is measured using a second instrument without error. What are the valid ranges of organic aerosol mass that will provide closure with kv and uncertainties associated with the AMS-kv calculation?
	
	$p2	
	
	If the externally measured κᵥ = 0.5: $(round(xvals[i2], digits = 2)) < forg < $(round(xvals[i1], digits = 2))

	If the externally measured κᵥ = 0.3: $(round(xvals[j2], digits = 2)) < forg < $(round(xvals[j1], digits = 2))
