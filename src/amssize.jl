...
gette(Dp) = @match Dp begin
	if Dp < 34.0 end   => 0.0
	if Dp > 1175.0 end => 0.0
	if Dp <= 1175.0 && Dp >= 482.0 end => tfun(1175.0, 482.0, Dp)
	if Dp <= 74.0 && Dp >= 34.0 end => tfun(34.0, 74.0, Dp) 
	_ => 1.0
end
...
ρ = 1.77
f = map(gette, 𝕟.Dp)
df1 = DataFrame(Dp=𝕟.Dp .*    ρ, S=𝕟.S * π .* (𝕟.Dp .* 1e-3).^3.0 ./ 6.0 * ρ, Distribution="True")
df2 = DataFrame(Dp=𝕟.Dp .* ρ, S=𝕟.S * π .* (𝕟.Dp .* 1e-3).^3.0 ./ 6.0 * ρ .* f, Distribution="Atten.")
colors = ["black", "darkred", "steelblue3", "darkgoldenrod", "grey"]

xlabels = log10.([30, 100, 300, 1000])
xticks = log10.([30, 40, 50, 60, 70, 80, 90, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000])
p0 = plot(
	x=𝕟.Dp, 
	y=𝕟.S,
	Geom.line,
...
draw(PNG("amssize.png", dpi=600), p)
